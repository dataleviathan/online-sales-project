...
options(warn=-1) library(sqldf) consumer<-read.csv("consumer.csv",header=TRUE) pop_up<- read.csv("pop_up.csv",header=TRUE) purchase<-read.csv("purchase.csv",header=TRUE) email<-read.csv("email.csv",header=TRUE)

#Exploring various tables

#Display first 5 rows of all the four tables

sqldf("SELECT * FROM consumer LIMIT 5;") sqldf("SELECT * FROM pop_up LIMIT 5;") sqldf("SELECT * FROM purchase LIMIT 5;") sqldf("SELECT * FROM email LIMIT 5;")
...
#The number of female consumers is roughly three times the number of male consumers. #The average age of male consumers is slightly higher than the female but they're both around 30.
...
#Half of the consumers did not receive the pop up. #Roughly half of those who received also added the discount code.
...
#Pop ups don't really help to increase sales because both the sum #and average sales are higher with those who did not receive pop ups.
...
sqldf(" SELECT consumer_id, opened_email FROM email WHERE consumer_id='5955534353' ")
